...
public class UserController {
    ...
    public String listByGroupName(@PathVariable("id") long id, ModelMap map) {  	
    	User u;
    	String groupName = null;
        User loggedInUser = userService.getLoggedInUser();
        if(id == 0) {
           groupName = loggedInUser.getGroupName();
        }
        Iterable<User> users = this.userRepository.findAllByGroupName(groupName);
        map.addAttribute("users", users);
        return "user/group";
    }
    ...
    //@RequestMapping(value = "/user/send-user-details", method = RequestMethod.POST)
